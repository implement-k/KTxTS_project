...
백엔드는 현재 브랜치로 모델 로딩과 호출 인터페이스를 연동할 수 있다. 가변-node 모델
코드는 수정됐지만 호환 체크포인트는 아직 확인하지 못했다. 현재 어댑터는
`feature/manage-version`의 고정 1,137-node `mae:v7.pth`를 기준으로 하며, 현재 버전은
백엔드 입력·출력 인터페이스 확인용이다. 새 호환 체크포인트를 받으면 내부 모델 연결부를
교체하고 재검증할 예정이다.
...
| AI | 확정 zone·node 순서·feature·scaler·연령/인구 정책과 가변-node 호환 체크포인트 전달 |
...
- 수정된 가변-node 모델 코드와 호환되는 체크포인트의 확인·전달 및 연동 검증
...
가변-node 모델 코드는 수정됐지만 호환 체크포인트는 아직 확인하지 못했다. 현재 어댑터에
연결된 `feature/manage-version`의 `mae:v7.pth`는 실제 검증 결과 1,137개 node에
고정돼 있다. 새 호환 체크포인트를 받으면 내부 모델 연결부를 교체하고 재검증할
예정이다.
...
가변-node 모델 코드는 수정됐지만 호환 체크포인트는 아직 확인하지 못했다. 새 체크포인트를
받으면 runner의 내부 모델 연결부를 교체하고 strict loading과 가변 N forward를 재검증해야
한다.
