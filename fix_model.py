...
# 1. Update DoublyConstrainedGravityModel.__init__
init_old = """    def __init__(self, beta=2.0, max_iter=100, tol=1e-4, generation_model_type='lgbm'):
        \"\"\"
            이중제약 중력모델 초기화
...
init_new = """    def __init__(self, generation_model_type='lgbm', beta=2.0, max_iter=100, tol=1e-4):
        self.generation_model_type = generation_model_type
        self.beta = beta
        self.max_iter = max_iter
        self.tol = tol
...
# 2. Update fit_lgbm_O_D
fit_od_old = """    def fit_lgbm_O_D(self, X_static, O_true, D_true):
        \"\"\"
...
fit_od_new = """    def fit_lgbm_O_D(self, X_static, O_true, D_true):
        \"\"\"
...
content = content.replace(fit_od_old, fit_od_new)

# 3. Update predict_O_D
pred_od_old = """    def predict_O_D(self, X_static):
        \"\"\"
        학습된 모델로 O_i, D_j 예측
        \"\"\"
...
pred_od_new = """    def predict_O_D(self, X_static):
        \"\"\"
...
content = content.replace(pred_od_old, pred_od_new)


# 4. Remove fit_lgbm_self_inter, predict_self_inter, predict_self
import re
content = re.sub(r'    def fit_lgbm_self_inter.*?    def apply_ipf', '    def apply_ipf', content, flags=re.DOTALL)


# 5. Update apply_ipf
apply_ipf_old = """    def apply_ipf(self, O_pred, D_pred, dist_matrix, y_self=None, y_inter=None):
        \"\"\"
...
apply_ipf_new = """    def apply_ipf(self, O_pred, D_pred, dist_matrix):
        \"\"\"
...
content = content.replace(apply_ipf_old, apply_ipf_new)

# 6. Update fit_predict
fit_predict_old = """    def fit_predict(self, X_static_train, O_train, D_train, X_static_all, dist_matrix, X_self=None, X_inter=None):
        \"\"\"
        [4단계] 전체 파이프라인 통합 실행.
...
fit_predict_new = """    def fit_predict(self, X_static_train, O_train, D_train, X_static_all, dist_matrix):
        \"\"\"
        [4단계] 전체 파이프라인 통합 실행.
        \"\"\"
...
content = content.replace(fit_predict_old, fit_predict_new)
