...
def evaluate_and_report(base_data, val_meta, model, year_label, split_name,  n_workers=4, device=None):
    ...
    # Save raw results for calibration
    if split_name == 'val' and records:
        import json
        raw_results = []
        for r in records:
            if 'raw_y_od' in r and 'raw_y_pred' in r:
                for a, p in zip(r['raw_y_od'], r['raw_y_pred']):
                    raw_results.append({'task': r['task'], 'y_actual': a, 'y_pred': p})
                
        if raw_results:
            out_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'mae_backend_adapter', 'calibration')
            os.makedirs(out_path, exist_ok=True)
            with open(os.path.join(out_path, 'validation_raw_results.json'), 'w') as f:
