...
def _eval_one_sample(args):
    ...
    try:
        ...
        if len(y_od_eval) > 0:
            ...
            vol_ratio = np.sum(y_pred_eval) / np.sum(y_od_eval) if np.sum(y_od_eval) > 0 else 0.0
        else:
            rmse_eval = cpc_eval = prmse_eval = vol_ratio = 0.0
            
        # self-loop vs ext 분리
        diag_mask = np.eye(N, dtype=bool)
        valid_self = valid_cells & diag_mask
        valid_ext = valid_cells & ~diag_mask
        
        def calc_sub_metrics(v_mask):
            vt = y_od[v_mask]
            vp = np.maximum(T_pred[v_mask], 0)
            if len(vt) > 0:
                r = np.sqrt(np.mean((vt - vp)**2))
                c = 2*np.sum(np.minimum(vt, vp)) / (np.sum(vt) + np.sum(vp) + 1e-9)
                return r, c
            return 0.0, 0.0
            
        rmse_self, cpc_self = calc_sub_metrics(valid_self)
        rmse_ext, cpc_ext = calc_sub_metrics(valid_ext)
        ...
        return {'year': year_label, 'city': city_name, 'task': task,
                ...
                'rmse_self': rmse_self, 'cpc_self': cpc_self,
                'rmse_ext': rmse_ext, 'cpc_ext': cpc_ext,
                'vol_ratio': vol_ratio, 'top20_acc': top20_acc,
                'split': split_name,
        ...
        for done_count, future in enumerate(as_completed(futures), start=1):
            ...
            if done_count % progress_every == 0 or done_count == total:
                ...
                if done_records:
                    ...
                    top20_mean = np.mean([r['top20_acc'] for r in done_records])
                    metric_text = (
...
def summarize_results(records, group_keys, label):
    ...
    for record in records:
        ...
        for metric in ('rmse', 'cpc', 'prmse', 'top20_acc', 'rmse_self', 'cpc_self', 'rmse_ext', 'cpc_ext', 'vol_ratio'):
            if metric in record:
    ...
    for key in sorted(grouped.keys()):
        ...
        vol_mean = np.mean(grouped[key].get('vol_ratio', [0.0]))
        cpc_s = np.mean(grouped[key].get('cpc_self', [0.0]))
        cpc_e = np.mean(grouped[key].get('cpc_ext', [0.0]))
        
        print(f"[{key_str}] (n={n}) "
              f"CPC={cpc_mean:.4f} (Self:{cpc_s:.4f} Ext:{cpc_e:.4f})  "
              f"RMSE={rmse_mean:.4f}  "
              f"VolRatio={vol_mean:.4f}{top20_str}")