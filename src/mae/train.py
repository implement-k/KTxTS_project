...
# ─────────────────────────────────────────────────────────────────────────────
# K-Fold 생성 (twostage와 동일: 지리적으로 가까운 동끼리 그룹화)
# ─────────────────────────────────────────────────────────────────────────────
...
def validate(model, dataset, val_indices, criterion, device):
    ...
    X_static_masked[val_indices, -1] = 1.0  # is_masked indicator
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    print("Starting MAE K-Fold Training...")
    ...
    for fold in range(args.k_fold):
        ...
        model     = SpatialODMAE(num_nodes=dataset.num_nodes,
                                  num_features=dataset.X_static.shape[1]).to(device)
        ...
        for epoch in range(args.epochs):
            ...
            pbar = tqdm(train_loader,
                        desc=f"Fold {fold+1} Epoch {epoch+1}/{args.epochs} "
                             f"[Mask:{current_mask_size} α:{current_alpha:.1f}]")
            for batch in pbar:
