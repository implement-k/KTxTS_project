...
def validate_mae(model, dataset, val_indices, criterion, device):
    ...
    # 100% masking for val and test nodes
    hide_mask_np = np.zeros(dataset.num_nodes, dtype=bool)
    if hasattr(dataset, 'test_indices'):
        hide_mask_np[dataset.test_indices] = True
    input_mask_np = val_mask_np | hide_mask_np

    X_static_masked = dataset.X_static.copy()
    if np.any(input_mask_np):
        X_static_masked[np.ix_(input_mask_np, dataset.masking_indices)] = 0.0
    X_static_masked[input_mask_np, -2] = 1.0
    X_static_masked[input_mask_np, -1] = 0.0

    X_OD_masked = dataset.X_OD.copy()
    X_OD_masked[input_mask_np, :] = 0
    X_OD_masked[:, input_mask_np] = 0
    ...
    is_hybrid_od = criterion.__class__.__name__ == 'HybridWeightedODLoss'
    ...
    if is_weibull:
        ...
        v_loss = criterion(v_pred_scale, v_pred_raw, y_o, 1.0, m2d, diag_mask, active_node_mask=active_node_mask).item()
        p_real = np.maximum(torch.expm1(v_pred_scale[m2d]).cpu().numpy(), 0)
    else:
...
def validate_mae_merge(model, dataset, val_indices, criterion, device):
    ...
    for i, (idx_a, idx_b) in enumerate(selected_pairs):
        ...
        # idx_a는 예측 타겟이므로 마스킹 대상임
        mask[idx_a] = True
        # 참고: idx_b는 Virtual Deletion 대상이므로 active_mask에서 제외되며, 
        # 모델의 예측 평가 대상이 아니므로 mask[idx_b] = False 로 유지됨.
        ...
        X_static_masked[idx_a, dataset.masking_indices] = 0.0
        X_static_masked[idx_a, -2] = 1.0
        X_static_masked[idx_a, -1] = 1.0
        
        # Virtual Deletion
        active_mask[idx_b] = False
        X_static_masked[idx_b, :-2] = 0.0
        X_static_masked[idx_b, -2] = 1.0
    ...
    if is_weibull:
        ...
        v_loss = criterion(v_pred_scale, v_pred_raw, y_o, 1.0, m2d, diag_mask, active_node_mask=active_node_mask).item()
        p_real_full = np.maximum(torch.expm1(v_pred_scale).cpu().numpy(), 0)
    else:
        v_loss = criterion(v_pred_raw, y_o, 1.0, m2d).item()
    ...
    return v_loss, rmse, cpc
def validate_mae_merge_partial(model, dataset, val_indices, criterion, device):
    """
    Task C: 병합 노드가 Context로 주어질 때(0, 1), 타겟 노드(val_indices)들의 예측 성능
    """
    if not hasattr(dataset, 'merge_cache') or len(dataset.merge_cache) == 0:
        return 0.0, 0.0, 0.0
        
    val_set = set(val_indices)
    pairs = [k for k in dataset.merge_cache.keys() if k[0] not in val_set and k[1] not in val_set]
    
    if len(pairs) == 0:
        return 0.0, 0.0, 0.0
        
    rng = random.Random(42)
    selected_pairs = rng.sample(pairs, min(5, len(pairs)))
    ...
    for i, (idx_a, idx_b) in enumerate(selected_pairs):
        ...
        mask[val_mask_np] = True
        
        X_OD_masked[val_mask_np, :] = 0
        X_OD_masked[:, val_mask_np] = 0
        if np.any(val_mask_np):
            X_static_masked[np.ix_(val_mask_np, dataset.masking_indices)] = 0.0
        X_static_masked[val_mask_np, -2] = 1.0
    ...
    x_o = torch.tensor(X_o, dtype=torch.float32, device=device)
    y_o = torch.tensor(Y_o, dtype=torch.float32, device=device)
    mask_ts = torch.tensor(Mask, dtype=torch.bool, device=device)
    active_node_mask = torch.tensor(ActiveMask, dtype=torch.bool, device=device)
    ...
    with torch.no_grad():
        v_pred_scale, v_pred_raw = model(x_s, x_o, x_d, mask_ts, active_node_mask=active_node_mask)

    m2d = mask_ts.unsqueeze(1) | mask_ts.unsqueeze(2)
    diag_mask = torch.eye(N, device=device, dtype=torch.bool).unsqueeze(0).expand(B, -1, -1)
...
def validate_mae_mask_with_known(model, dataset, val_indices, criterion, device):
    """
    Task D: 마스킹 동 + 알려진 동 병합 (mask_with_known)
    """
    if not hasattr(dataset, 'merge_cache') or len(dataset.merge_cache) == 0:
        return 0.0, 0.0, 0.0
        
    val_set = set(val_indices)
    pairs = [k for k in dataset.merge_cache.keys() if (k[0] in val_set and k[1] not in val_set) or (k[0] not in val_set and k[1] in val_set)]
    ...
    for i, (idx_a, idx_b) in enumerate(selected_pairs):
        ...
        known_node, val_node = (idx_a, idx_b) if idx_b in val_set else (idx_b, idx_a)
        
        # val_node는 병합되어 사라지므로 예측(마스킹) 대상에서 제외
        curr_val_mask_np = np.zeros(N, dtype=bool)
        curr_val_mask_np[val_indices] = True
        curr_val_mask_np[val_node] = False
        
        mask[curr_val_mask_np] = True
        
        X_OD_masked[curr_val_mask_np, :] = 0
        X_OD_masked[:, curr_val_mask_np] = 0
        if np.any(curr_val_mask_np):
            X_static_masked[np.ix_(curr_val_mask_np, dataset.masking_indices)] = 0.0
        X_static_masked[curr_val_mask_np, -2] = 1.0
        X_static_masked[curr_val_mask_np, -1] = 0.0
        
        # known_node에 병합된 결과 덮어쓰기
        cache_key = (known_node, val_node)
        if cache_key not in dataset.merge_cache:
            cache_key = (val_node, known_node)
        cache = dataset.merge_cache[cache_key]
        
        raw_od_known = np.expm1(y_OD[known_node, :])
        raw_od_val = np.expm1(y_OD[val_node, :])
        new_self_loop = (
            np.expm1(y_OD[known_node, known_node]) + np.expm1(y_OD[val_node, val_node]) +
            np.expm1(y_OD[known_node, val_node]) + np.expm1(y_OD[val_node, known_node])
        )
        
        raw_y_od_row = raw_od_known + raw_od_val
        raw_y_od_col = np.expm1(y_OD[:, known_node]) + np.expm1(y_OD[:, val_node])
        
        y_OD[known_node, :] = np.log1p(raw_y_od_row)
        y_OD[:, known_node] = np.log1p(raw_y_od_col)
        y_OD[known_node, known_node] = np.log1p(new_self_loop)
        
        merged_dist = cache['merged_dist_row_at_a']
        X_dist_curr[known_node, :] = np.log1p(merged_dist)
        X_dist_curr[:, known_node] = np.log1p(merged_dist)
        
        merged_static_scaled = dataset.scaler.transform(cache['merged_raw_static_at_a'].reshape(1, -1))[0]
        X_static_masked[known_node, :-2] = merged_static_scaled
        X_static_masked[known_node, -2] = 0.0
        X_static_masked[known_node, -1] = 1.0 # 근사치 Context
        
        # val_node (secondary) 삭제
        active_mask[val_node] = False
        X_static_masked[val_node, :-2] = 0.0
        X_static_masked[val_node, -2] = 1.0
        X_static_masked[val_node, -1] = 0.0
        X_OD_masked[val_node, :] = 0.0
        X_OD_masked[:, val_node] = 0.0
        X_dist_curr[val_node, :] = 5.5
        X_dist_curr[:, val_node] = 5.5
        
        # Context로 쓰인 known_node의 정답 유출 방지 (마스킹이 풀려있지만 OD를 0으로 덮어씌움)
        X_OD_masked[known_node, :] = y_OD[known_node, :]
        X_OD_masked[:, known_node] = y_OD[:, known_node]
        
        # 다시 curr_val_mask_np로 0 만들기 (known_node 덮어씌우면서 침범했을 수 있으므로)
        X_OD_masked[curr_val_mask_np, :] = 0
        X_OD_masked[:, curr_val_mask_np] = 0
