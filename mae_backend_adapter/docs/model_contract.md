...
| `x_od_masked` | `(B, N, N)` | `float32` | `log1p` OD, mask 행/열은 0 |
...
현재 운영 추론은 선택한 신도시 node만 `mask=True`로 설정하고,
mask된 node의 OD 행과 열을 0으로 만든다.
...
Adapter에 포함된 `model/mae.py::ODMAE`를 생성한 후 `model/mae.pth`를
`strict=True`로 load한다. 모델은 self-loop를 포함한 전체 OD를 직접 예측하며
별도 LightGBM 보정을 사용하지 않는다.

## 5. 과거 v7 Adapter와의 차이

| 구분 | 과거 v7 | 현재 mae-year |
|---|---|---|
| 모델 class | `SpatialODMAE` | `ODMAE` |
| Forward | 4개 tensor | 5개 tensor |
| Static feature | 15 | 20 |
| Node 수 | decoder에서 고정값 추론 | `x_static.shape[0]`에서 요청마다 결정 |
| 추가 입력 | 없음 | `a_spatial`, `mask` |
| 중복 code | 거부 | OD 합산 |
