...
def _coerce_numeric_raw_static(raw_static, expected_len):
    ...
    if len(numeric_values) >= expected_len:
        arr = np.asarray(numeric_values[-expected_len:], dtype=np.float32)
    ...
    def __getitem__(self, idx):
        ...
        for idx_a, idx_b, event_type in merge_events:
            ...
            if event_type == 'known_merge':
                ...
                X_static_raw_masked[primary_node, :-2] = merged_raw_static
                # leak 수정: known 동의 원본 + 0 (마스킹)
                X_static_raw_masked[primary_node, self.masking_indices] = (
                    self.X_static_raw[primary_node, self.masking_indices]
                )
                corrected_raw = X_static_raw_masked[primary_node, :-2]
                
                # 수정된 raw 값을 다시 scaler로 transform 하여 scaled 버전에 반영
                corrected_scaled = self.scaler.transform(corrected_raw.reshape(1, -1))[0]
